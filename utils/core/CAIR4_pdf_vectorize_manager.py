...
ACHTUNG: import pymupdf as fitz ist wichtig, da ansonsten Konflikte mit "frontend" auftauchen können.
...
CONTEXT_WINDOW = 250  # Anzahl der Zeichen vor und nach dem Keyword
...
def create_faiss_index():
    ...
    st.info(" Indexierung wird gestartet...")
    ...
    st.success("✅ PDF-Index erfolgreich erstellt.")

def render_pdf_search_results(query, top_k=10, min_score=0.0):
    if not os.path.exists(CACHE_DIR):
        os.makedirs(CACHE_DIR)

    if not os.path.exists(INDEX_CACHE) or not os.path.exists(METADATA_CACHE):
        create_faiss_index()
        st.info("Index wurde erstellt.")
    ...
    try:
        ...
        # === 🧠 Query Expansion mit Varianten
        query_variants = [query]
        if "-" in query:
            parts = query.split("-")
            query_variants += parts
            query_variants.append(" ".join(parts))    # z. B. "KI System"
            query_variants.append("".join(parts))     # z. B. "KISystem"
        ...
        for idx, score in zip(I[0], D[0]):
            if score > min_score:
                ...
                # Sicherstellen, dass das Query irgendwo im Chunk vorkommt
                if not re.search(re.escape(query), chunk, flags=re.IGNORECASE):
                    continue

                key = f"{meta['pdf']}_{meta['page']}_{chunk[:30]}"
                if key in seen_keys:
                    continue
                seen_keys.add(key)
                ...
                if keyword_positions:
                    ...
                    highlighted_context = "Kein Kontext gefunden."
        ...
        if not results:
            st.info("🚫 Keine semantischen Treffer gefunden.")
            return

        # === Gruppiere Ergebnisse nach PDF-Datei ===
        ...
        for pdf_name, entries in grouped.items():
            ...
            with st.expander(f"📄 {pdf_name} ({kapitel}) – {len(entries)} Treffer"):
                ...
                st.markdown(f'<a href="{view_url}">📂 Öffnen im PDF-Viewer</a>', unsafe_allow_html=True)

    except Exception as e:
        st.error(f"❌ Fehler beim Laden der PDF-Suchmoduls: {e}")

# === Haupt-Renderfunktion ===
def render_pdf_vectorizer_view():
    st.markdown(
        '[🧪 PDF öffnen](http://localhost:8501/static/pdf_results/CAIR4_about.pdf)',
        unsafe_allow_html=True
    )
    st.subheader(" Lokale PDF-Suche mit FAISS")

    if st.button(" Index jetzt erstellen", type="primary"):
        create_faiss_index()

    query = st.text_input(" Was möchtest du wissen?")
    if query:
        render_pdf_search_results(query)
